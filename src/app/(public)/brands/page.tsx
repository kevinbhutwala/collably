      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {brands.map((brand) => (
            <div
              key={brand.id}
              className="rounded-3xl bg-white border border-slate-200 hover:border-slate-300 p-6 shadow-card hover:shadow-elevated transition-all flex flex-col justify-between"
            >
              <div className="space-y-4">
                <div className="flex items-center gap-3.5">
                  <div className="w-14 h-14 rounded-2xl bg-slate-900 border border-slate-800 text-white flex items-center justify-center shrink-0 shadow-md">
                ...
                <p className="text-xs text-slate-800 font-semibold line-clamp-1">{brand.headline}</p>
                <p className="text-xs text-slate-600 line-clamp-2 leading-relaxed">{brand.description}</p>

                <div className="grid grid-cols-2 gap-2 p-3 rounded-2xl bg-slate-50 border border-slate-100 font-mono text-center text-xs">
                  <div>
                    <span className="text-[10px] text-slate-400 block uppercase">Campaigns</span>
                    <span className="font-bold text-slate-900">{brand.activeCampaignsCount} Active</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block uppercase">Escrow Volume</span>
                    <span className="font-bold text-emerald-600">{formatCurrency(brand.totalSpent)}</span>
                  </div>
                </div>
              </div>
