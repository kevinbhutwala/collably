...
export default function CampaignsDirectoryPage() {
  ...
  return (
    <div className="py-12 sm:py-16 bg-[#FAFAFC] text-[#0A0A0E] min-h-screen select-none space-y-12">
      ...
      {/* Overlapping Campaign Cards Grid with 3D Tilt */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map((n) => (
              <div key={n} className="h-64 rounded-3xl bg-white border border-black/6 animate-pulse" />
            ))}
          </div>
        ) : campaigns.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-black/8 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {campaigns.map((camp) => (
              <InteractiveTiltCard
                ...
                className="rounded-3xl bg-white border border-black/8 hover:border-black/20 shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_16px_40px_rgba(0,0,0,0.08)] transition-all p-6 flex flex-col justify-between group relative"
              >
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <span className="text-[10px] font-mono text-[#888898] uppercase block">Verified Brand</span>
                        <span className="text-xs font-bold text-[#0A0A0E] font-display">{camp.category}</span>
                      </div>
                    </div>
                  ...
                  {/* Overlapping Visual Deliverables Chip Bar */}
                ...
                <div className="pt-4 mt-2">
                  <Link
                    ...
                    className="w-full py-2.5 rounded-full bg-[#FAF9F5] hover:bg-[#FFD21F] text-[#0A0A0E] font-sans font-bold text-xs transition-colors flex items-center justify-center gap-1.5 border border-black/8 shadow-2xs"
                  >
