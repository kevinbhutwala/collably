      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-6 p-6 sm:p-8 rounded-3xl bg-[#120c16] border border-white/10 shadow-card space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold text-white font-display">Creator Verification Queue</h3>
              <p className="text-xs text-slate-400 font-sans">Review audience authenticity and toggle verified badges.</p>
            </div>
            ...
            {creators.slice(0, 5).map((c) => (
              <div key={c.id} className="py-4 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="relative w-10 h-10 rounded-xl overflow-hidden bg-white/[0.05] border border-white/10 shrink-0">
                    <SafeImage
                      src={c.avatarUrl}
                      alt={c.fullName}
        ...
        <div className="lg:col-span-6 p-6 sm:p-8 rounded-3xl bg-[#120c16] border border-white/10 shadow-card space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold text-white font-display">Escrow Settlement Vault</h3>
              <p className="text-xs text-slate-400 font-sans">Funds secured in platform vault for active campaigns.</p>
            </div>
            <span className="text-xs font-mono text-emerald-400 font-bold">100% Solvency</span>
          </div>

          <div className="space-y-3 font-mono text-xs">
            {campaigns.map((camp) => (
              <div key={camp.id} className="p-4 rounded-2xl bg-white/[0.03] border border-white/10 flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-white font-sans text-xs">{camp.title}</h4>
                  <p className="text-slate-400 font-sans text-[11px]">{camp.brand.companyName}</p>
                </div>
                <div className="text-right">
                  <span className="text-emerald-400 font-bold block">{formatCurrency(camp.budget.totalBudget)}</span>
