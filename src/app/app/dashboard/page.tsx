    ...
    <div className="space-y-10 text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Link
              ...
              className="text-xs text-[hsl(327,100%,55%)] font-bold hover:underline font-mono"
            >
              View all &rarr;
            </Link>
          </div>
          ...
          {collaborations.length === 0 && role === "creator" ? (
            ...
            <div className="space-y-4">
              {activeCampaigns.slice(0, 3).map((c) => (
                <div
                  key={c.id}
                  className="p-6 rounded-3xl bg-[#120c16] border border-white/10 hover:border-[hsl(327,100%,50%)]/40 shadow-card hover:shadow-elevated transition-all duration-300 flex flex-col sm:flex-row sm:items-center justify-between gap-5 group relative overflow-hidden"
                >
                  {/* Subtle top gradient accent */}
                  <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[hsl(327,100%,50%)] to-[hsl(300,100%,42%)] opacity-0 group-hover:opacity-100 transition-opacity" />

                  <div className="flex items-start gap-4 flex-1">
                    <div className="w-12 h-12 rounded-2xl bg-white/[0.06] border border-white/10 text-white flex items-center justify-center shrink-0 shadow-md group-hover:scale-105 transition-transform">
                      <BrandIcon name={c.brand?.companyName || "Brand"} size={22} className="text-white" />
                    </div>

                    <div className="space-y-1.5 flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-black text-white font-display text-base group-hover:text-[hsl(327,100%,55%)] transition-colors truncate">
                          {c.title}
                        </span>
                        <span className="px-2.5 py-0.5 rounded-full bg-pink-500/15 border border-pink-500/30 text-pink-300 text-[10px] font-mono font-bold">
                          {c.category}
                        </span>
                      </div>
                  ...
                  <div className="flex items-center gap-4 shrink-0 font-mono pt-2 sm:pt-0 border-t sm:border-t-0 border-white/10 justify-between sm:justify-end">
                    ...
                    <Link href={`/campaigns/${c.id}`}>
                      <button className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-[hsl(327,100%,50%)] to-[hsl(300,100%,42%)] hover:brightness-110 text-white font-bold text-xs font-display transition-all flex items-center gap-1.5 shadow-md shadow-pink-500/20">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          {/* Escrow Trust Assurance Box (anchorhetvi.com dark velvet style) */}
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#180f1d] via-[#120c16] to-[#0c0810] border border-pink-500/25 hover:border-pink-500/40 shadow-card hover:shadow-elevated transition-all duration-300 p-6 space-y-4 text-white">
            {/* Luminous Top Accent */}
            <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[hsl(327,100%,50%)] via-purple-500 to-emerald-400" />

            {/* Header */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[hsl(327,100%,50%)] to-purple-600 text-white flex items-center justify-center shadow-md shadow-pink-500/25">
                  <ShieldCheck className="w-5 h-5" />
                </div>
                <div>
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[hsl(327,100%,55%)] block">
              ...
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-[10px] font-mono font-bold">
            ...
            <div className="pt-3 border-t border-white/10 flex items-center justify-between text-[11px] font-mono">
              ...
              <span className="text-slate-200 font-bold flex items-center gap-1">
                <span className="text-emerald-400">●</span> Razorpay / Stripe
              </span>
            </div>
          </div>
        </div>
