...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8 text-white font-sans select-none">
      ...
      <div className="rounded-3xl bg-gradient-to-r from-[#0C0C14] via-[#1A1810] to-[#2B240E] border border-[#FFD21F]/30 p-6 sm:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.6),0_0_35px_rgba(255,210,31,0.18)] backdrop-blur-xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2 relative z-10 max-w-xl">
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#FFD21F]/15 border border-[#FFD21F]/30 text-[11px] font-mono font-extrabold text-[#FFD21F] shadow-inner">
              <span className="w-2 h-2 rounded-full bg-[#FFD21F] animate-pulse" />
              Live Workspace Active
            </span>
            <span className="px-2.5 py-1 rounded-full bg-white/10 border border-white/20 text-white text-[10px] font-mono font-bold uppercase tracking-wider">
              {role.replace(/_/g, " ")}
            </span>
          </div>

          <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight font-display">
            Welcome back, <span className="bg-gradient-to-r from-white via-[#FFF3B0] to-[#FFD21F] bg-clip-text text-transparent">{user?.name || "Collaborator"}</span>
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-3 relative z-10 font-sans">
          {role === "creator" ? (
            <>
              <Link href="/campaigns">
                <button className="px-5 py-3 rounded-full bg-gradient-to-r from-[#FFD21F] via-[#FFE052] to-[#FFC700] hover:from-[#FFE052] hover:to-[#FFD21F] text-[#0A0A0E] font-bold text-xs transition-all flex items-center gap-2 shadow-[0_0_25px_rgba(255,210,31,0.45)] border border-white/50 group">
                  <Compass className="w-4 h-4 text-[#0A0A0E]" />
                  <span>Discover Briefs</span>
                </button>
              </Link>
              <Link href="/app/profile">
                <button className="px-5 py-3 rounded-full bg-white/10 hover:bg-white/15 border border-[#FFD21F]/30 text-white font-semibold text-xs transition-all flex items-center gap-2 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="rounded-3xl bg-gradient-to-br from-[#101018] to-[#08080C] border border-[#FFD21F]/20 p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-white/10">
              ...
              <Link
                ...
                className="text-xs font-mono font-bold text-[#FFD21F] hover:text-[#FFE052] transition-colors flex items-center gap-1 shrink-0"
              >
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                {collaborations.map((collab) => (
                  <div
                    key={collab.id}
                    className="p-5 rounded-2xl bg-gradient-to-r from-[#14141E]/90 to-[#0A0A10]/90 border border-white/10 hover:border-[#FFD21F]/40 transition-all space-y-3"
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#FFD21F]/20 to-white/10 border border-[#FFD21F]/30 flex items-center justify-center shrink-0">
                          <BrandIcon
                            name={collab.brand?.companyName || "Linear"}
                            className="w-5 h-5 text-[#FFD21F]"
                          />
                        </div>
                        <div>
                      ...
                      <span className="px-3 py-1 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider self-start sm:self-auto bg-[#FFD21F]/20 text-[#FFD21F] border border-[#FFD21F]/40">
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-white/10 text-xs font-mono">
                      ...
                      <Link
                        href="/app/collaborations"
                        className="text-[#FFD21F] hover:text-white font-bold flex items-center gap-1 transition-colors"
                      >
          ...
          <div className="rounded-3xl bg-gradient-to-br from-[#101018] to-[#08080C] border border-[#FFD21F]/20 p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-white/10">
              ...
              <Link
                href="/campaigns"
                className="text-xs font-mono font-bold text-[#FFD21F] hover:text-[#FFE052] transition-colors flex items-center gap-1"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {activeCampaigns.slice(0, 4).map((c) => (
                <Link
                  key={c.id}
                  href={`/campaigns/${c.id}`}
                  className="p-5 rounded-2xl bg-gradient-to-r from-[#14141E]/90 to-[#0A0A10]/90 border border-white/10 hover:border-[#FFD21F]/40 transition-all group flex flex-col justify-between space-y-3"
                >
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <span className="px-2.5 py-0.5 rounded-full bg-[#FFD21F]/15 text-[#FFD21F] text-[10px] font-mono font-bold uppercase border border-[#FFD21F]/30">
                        {c.category}
                      </span>
                      <span className="text-[11px] font-mono text-white font-extrabold">
                        {formatCurrency(c.budget.totalBudget)} Pool
                      </span>
                    </div>
                    <h3 className="font-bold text-sm text-white group-hover:text-[#FFD21F] transition-colors font-display line-clamp-1">
                      {c.title}
                    </h3>
                    <p className="text-xs text-white/60 line-clamp-2 mt-1 font-sans">
                      {c.tagline}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-white/10 text-xs font-mono text-white/50">
                    ...
                    <span className="text-[#FFD21F] font-bold group-hover:translate-x-0.5 transition-transform flex items-center gap-0.5">
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="rounded-3xl bg-gradient-to-br from-[#101018] to-[#08080C] border border-[#FFD21F]/20 p-6 shadow-2xl backdrop-blur-xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <h3 className="text-sm font-bold text-white font-display">Recent Tranche Activity</h3>
              <Link
                href="/app/earnings"
                className="text-[11px] font-mono text-[#FFD21F] hover:text-white font-bold"
              >
                Ledger
              </Link>
            </div>
                ...
                {recentPayouts.slice(0, 3).map((p) => (
                  <div key={p.id} className="py-3 flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-[10px] text-white/50">{p.brandName}</span>
                    </div>
                    <div className="text-right">
                      <span className="font-bold text-[#FFD21F] block">
                        +{formatCurrency(p.netAmount)}
                      </span>
                      <span className="text-[9px] text-white/50 uppercase">{p.status}</span>
                    </div>
                  </div>
                ))}
              </div>
