...
export default function ApplicationsManagementPage() {
  ...
  return (
    <div className="space-y-6 text-[#0A0A0E] dark:text-[#F4F4F8] select-none font-sans">
      ...
      <div className="flex items-center gap-1.5 p-1 bg-[#F4F4F8] dark:bg-[#12121A] rounded-2xl border border-black/8 dark:border-white/10 overflow-x-auto no-scrollbar shadow-xs">
        {tabs.map((tab) => {
            ...
            <button
              ...
              className={cn(
                ...
                isActive
                  ...
                  : "text-[#5A5A68] dark:text-[#A0A0B4] hover:text-[#0A0A0E] dark:hover:text-[#F4F4F8] hover:bg-black/5 dark:hover:bg-white/5"
              )}
            >
      ...
      {filtered.length === 0 ? (
        <div className="py-16 text-center rounded-3xl bg-white dark:bg-[#12121A] border border-black/8 dark:border-white/10 p-6 space-y-2 shadow-xs">
          <p className="text-sm font-bold text-[#0A0A0E] dark:text-[#F4F4F8]">No applications found</p>
          <p className="text-xs text-[#6A6A78] dark:text-[#8E8EA4]">No items currently match the selected status filter.</p>
        </div>
      ) : (
        <div className="space-y-3.5">
          {filtered.map((app) => (
            <div
              key={app.id}
              className="p-5 sm:p-6 rounded-3xl bg-white dark:bg-[#12121A] border border-black/8 dark:border-white/10 shadow-xs flex flex-col lg:flex-row lg:items-center justify-between gap-4 text-[#0A0A0E] dark:text-[#F4F4F8]"
            >
              <div className="space-y-2.5 flex-1">
                <div className="flex items-center gap-3">
                  <div className="relative w-10 h-10 rounded-2xl overflow-hidden bg-[#F5F5F9] dark:bg-[#181824] border border-black/8 dark:border-white/10 shrink-0">
                    <SafeImage
                      src={app.brandLogo}
                      alt={app.brandName}
                      fallbackType="brand"
                  ...
                  <div className="min-w-0">
                    <h3 className="font-bold text-sm sm:text-base text-[#0A0A0E] dark:text-white truncate font-display">{app.campaignTitle}</h3>
                    <p className="text-xs text-[#6A6A78] dark:text-[#8E8EA4] font-sans truncate">
                  ...
                  <span className={`ml-auto px-2.5 py-1 rounded-full text-[10px] font-sans font-extrabold uppercase tracking-wider shrink-0 ${
                    app.status === "accepted" ? "bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800" : "bg-black/5 dark:bg-white/10 text-[#5A5A68] dark:text-[#A0A0B4]"
                  }`}>
                    {app.status}
                  </span>
                </div>

                <div className="p-3.5 rounded-2xl bg-[#F8F8FC] dark:bg-[#181824] border border-black/5 dark:border-white/10 text-xs text-[#4A4A58] dark:text-[#D0D0E0] leading-relaxed">
                  <strong className="text-[#0A0A0E] dark:text-[#F4F4F8] block mb-0.5 text-xs font-bold">Pitch Statement:</strong>
                  <p className="line-clamp-2">{app.pitch}</p>
                </div>
              </div>

              <div className="flex lg:flex-col items-center lg:items-end justify-between gap-3 shrink-0 pt-2 lg:pt-0 border-t lg:border-t-0 border-black/6 dark:border-white/10">
                <div>
                  <span className="text-[10px] text-[#7A7A8A] dark:text-[#8E8EA4] font-bold uppercase block">Proposed Fee</span>
                  <span className="text-base font-black text-[#0A0A0E] dark:text-white font-mono numeric-tabular">
                    {formatCurrency(app.proposedFee, app.currency || "USD")}
                    {(app.currency || "USD").toUpperCase() !== displayCurrency.toUpperCase() && (
                      <span className="text-[11px] font-semibold text-[#7A7A8A] dark:text-[#8E8EA4] font-mono block">
                ...
                {role === "brand" && app.status === "pending" && (
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-full bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/15 text-[#0A0A0E] dark:text-[#F4F4F8] text-xs font-bold transition-all border border-black/10 dark:border-white/10"
                    >
                      Decline
                    </button>
                    <button
                ...
                {app.status === "accepted" && (
                  <Link href="/app/collaborations">
                    <button className="px-4 py-2 rounded-full bg-black/5 dark:bg-white/10 hover:bg-[#FFD21F] dark:hover:bg-[#FFD21F] text-[#0A0A0E] dark:text-[#F4F4F8] dark:hover:text-[#0A0A0E] font-extrabold text-xs transition-all flex items-center gap-1.5 border border-black/10 dark:border-white/10">
