"use client";

import React, { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";

interface CreativeLoaderProps {
  label?: string;
  subtext?: string;
  size?: "sm" | "md" | "lg" | "fullscreen";
  className?: string;
  showProgress?: boolean;
}

const TELEMETRY_PHRASES = [
  "Synchronizing Escrow Milestones...",
  "Verifying Creator & Brand Channels...",
  "Calibrating 4K Review Rails...",
  "Securing Cryptographic Ledger...",
  "Establishing Instant Payout Rail...",
];

export function CreativeLoader({
  label,
  subtext,
  size = "md",
  className = "",
  showProgress = true,
}: CreativeLoaderProps) {
  const [phaseIndex, setPhaseIndex] = useState(0);

  useEffect(() => {
    const interval = setInterval(() => {
      setPhaseIndex((prev) => (prev + 1) % TELEMETRY_PHRASES.length);
    }, 2200);
    return () => clearInterval(interval);
  }, []);

  const isFullscreen = size === "fullscreen";

  // Micro loader variant (for buttons and inline widgets)
  if (size === "sm") {
    return (
      <div className={`inline-flex items-center gap-2 select-none ${className}`}>
        <div className="relative w-4 h-4 flex items-center justify-center">
          {/* Micro Orbit Track */}
          <motion.div
            animate={{ rotate: 360 }}
            transition={{ duration: 1.2, repeat: Infinity, ease: "linear" }}
            className="w-full h-full rounded-full border-2 border-[#FFD21F]/80 border-t-transparent"
          />
          {/* Center Deal Node */}
          <span className="w-1 h-1 rounded-full bg-[#FFD21F] shadow-[0_0_4px_#FFD21F]" />
        </div>
        {label && (
          <span className="text-xs font-mono font-medium text-[#0A0A0E] dark:text-[#F4F4F8] tracking-tight">
            {label}
          </span>
        )}
      </div>
    );
  }

  // Small, refined icon dimensions
  const iconDimensions = {
    md: "w-9 h-9 p-1.5 rounded-xl",
    lg: "w-11 h-11 p-2 rounded-2xl",
    fullscreen: "w-11 h-11 p-2 rounded-2xl",
  };

  return (
    <div
      className={
        isFullscreen
          ? `fixed inset-0 z-50 flex flex-col items-center justify-center p-6 bg-white/75 dark:bg-[#07070B]/85 backdrop-blur-xl select-none overflow-hidden transition-colors ${className}`
          : `flex flex-col items-center justify-center select-none py-6 px-4 text-center ${className}`
      }
      role="status"
      aria-live="polite"
    >
      {/* ── 1. Top Horizon Laser Ray (Fullscreen mode) ── */}
      {isFullscreen && (
        <div className="fixed top-0 left-0 right-0 h-0.5 bg-transparent overflow-hidden pointer-events-none z-50">
          <motion.div
            animate={{
              x: ["-100%", "100%"],
            }}
            transition={{
              duration: 1.6,
              repeat: Infinity,
              ease: [0.65, 0, 0.35, 1],
            }}
            className="w-1/3 h-full bg-gradient-to-r from-transparent via-[#FFD21F] to-transparent shadow-[0_0_8px_#FFD21F]"
          />
        </div>
      )}

      {/* ── 2. Small Refined Emblem with Subtle Halo ── */}
      <div className="relative flex items-center justify-center">
        {/* Soft, compact glow aura */}
        <motion.div
          animate={{
            scale: [0.95, 1.1, 0.95],
            opacity: [0.25, 0.45, 0.25],
          }}
          transition={{
            duration: 3,
            repeat: Infinity,
            ease: "easeInOut",
          }}
          className="absolute w-28 h-28 rounded-full bg-gradient-to-tr from-[#FFD21F]/25 to-transparent blur-xl pointer-events-none"
        />

        {/* Delicate thin orbit ring */}
        <motion.div
          animate={{ rotate: 360 }}
          transition={{ duration: 6, repeat: Infinity, ease: "linear" }}
          className="absolute -inset-2 rounded-full border border-dashed border-[#FFD21F]/40 pointer-events-none"
        >
          {/* Micro satellite dot */}
          <div className="w-1.5 h-1.5 rounded-full bg-[#FFD21F] shadow-[0_0_6px_#FFD21F] absolute -top-0.5 left-1/2 -translate-x-1/2" />
        </motion.div>

        {/* Compact Small Obsidian Emblem Tile (44px) */}
        <motion.div
          animate={{
            scale: [1, 1.04, 1],
            boxShadow: [
              "0 4px 14px -2px rgba(255, 210, 31, 0.25)",
              "0 6px 20px -2px rgba(255, 210, 31, 0.45)",
              "0 4px 14px -2px rgba(255, 210, 31, 0.25)",
            ],
          }}
          transition={{
            duration: 2,
            repeat: Infinity,
            ease: "easeInOut",
          }}
          className={`relative z-10 flex items-center justify-center bg-gradient-to-b from-[#181824] via-[#101018] to-[#0A0A0E] border border-[#FFD21F]/50 shadow-lg ${
            iconDimensions[size] || iconDimensions.md
          }`}
        >
          {/* Top Specular Sheen */}
          <div className="absolute inset-x-0 top-0 h-1/2 rounded-t-xl bg-gradient-to-b from-white/20 to-transparent pointer-events-none" />

          {/* Clean Small Abey Nexus SVG */}
          <svg
            viewBox="0 0 48 48"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="w-full h-full relative z-10"
          >
            <defs>
              <linearGradient id="creativeLoaderGoldSm" x1="12" y1="10" x2="36" y2="38" gradientUnits="userSpaceOnUse">
                <stop offset="0%" stopColor="#FFF275" />
                <stop offset="50%" stopColor="#FFD21F" />
                <stop offset="100%" stopColor="#FF9800" />
              </linearGradient>

              <linearGradient id="creativeLoaderBrandSm" x1="16" y1="18" x2="36" y2="36" gradientUnits="userSpaceOnUse">
                <stop offset="0%" stopColor="#FFFFFF" />
                <stop offset="100%" stopColor="#94A3B8" />
              </linearGradient>

              <radialGradient id="creativeCorePulseSm" cx="50%" cy="50%" r="50%">
                <stop offset="0%" stopColor="#FFE066" />
                <stop offset="60%" stopColor="#FFD21F" />
                <stop offset="100%" stopColor="#E6A800" />
              </radialGradient>
            </defs>

            {/* Creator Arch */}
            <motion.path
              d="M 12 37.5 L 21.8 12.2 C 22.7 9.8 25.3 9.8 26.2 12.2 L 36 37.5"
              stroke="url(#creativeLoaderGoldSm)"
              strokeWidth="4.5"
              strokeLinecap="round"
              strokeLinejoin="round"
              initial={{ pathLength: 0.2 }}
              animate={{
                pathLength: [0.3, 1, 0.3],
                pathOffset: [0, 0.5, 1],
              }}
              transition={{
                duration: 2.4,
                repeat: Infinity,
                ease: "easeInOut",
              }}
            />

            {/* Brand Synergy Loop */}
            <motion.path
              d="M 16.5 28.5 C 16.5 22.5 23 21 27.5 23.5 C 32 26 33.5 31.5 29.5 34 C 25.5 36.5 19.5 33 19.5 27 C 19.5 22.5 25.5 20.5 31.5 24"
              stroke="url(#creativeLoaderBrandSm)"
              strokeWidth="3.6"
              strokeLinecap="round"
              strokeLinejoin="round"
              initial={{ pathLength: 0.2 }}
              animate={{
                pathLength: [0.3, 1, 0.3],
                pathOffset: [1, 0.5, 0],
              }}
              transition={{
                duration: 2.8,
                repeat: Infinity,
                ease: "easeInOut",
              }}
            />

            {/* Pulsing Central Deal Core Node */}
            <motion.circle
              cx="24.5"
              cy="27.5"
              r="3.5"
              fill="url(#creativeCorePulseSm)"
              animate={{
                scale: [1, 1.3, 1],
                filter: [
                  "drop-shadow(0 0 2px #FFD21F)",
                  "drop-shadow(0 0 5px #FFD21F)",
                  "drop-shadow(0 0 2px #FFD21F)",
                ],
              }}
              transition={{
                duration: 1.2,
                repeat: Infinity,
                ease: "easeInOut",
              }}
            />
            <circle cx="24.5" cy="27.5" r="1.4" fill="#0A0A0E" />
          </svg>
        </motion.div>
      </div>

      {/* ── 3. Compact Minimal Telemetry ── */}
      <div className="relative z-10 space-y-2 max-w-xs px-4 pt-4">
        {/* Compact Chip with Equalizer */}
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/90 dark:bg-[#12121A]/90 border border-black/8 dark:border-white/10 shadow-2xs backdrop-blur-sm">
          <div className="flex items-center gap-0.5 h-2.5">
            {[0, 1, 2].map((i) => (
              <motion.span
                key={i}
                animate={{
                  height: ["25%", "100%", "35%"],
                }}
                transition={{
                  duration: 0.7 + i * 0.18,
                  repeat: Infinity,
                  ease: "easeInOut",
                }}
                className="w-0.5 rounded-full bg-[#FFD21F]"
              />
            ))}
          </div>

          <span className="text-[10px] font-mono font-bold tracking-wider uppercase text-[#0A0A0E] dark:text-white">
            {label || "ABEYCOLLAB"}
          </span>
        </div>

        {/* Phase Subtitle */}
        <div className="h-4 overflow-hidden flex items-center justify-center">
          <AnimatePresence mode="wait">
            <motion.p
              key={phaseIndex}
              initial={{ opacity: 0, y: 4 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -4 }}
              transition={{ duration: 0.2, ease: "easeOut" }}
              className="text-[11px] font-medium text-[#6A6A78] dark:text-[#9A9AB0] font-sans truncate"
            >
              {subtext || TELEMETRY_PHRASES[phaseIndex]}
            </motion.p>
          </AnimatePresence>
        </div>

        {/* Micro Shimmer Track */}
        {showProgress && (
          <div className="w-28 sm:w-36 h-0.5 rounded-full bg-black/8 dark:bg-white/10 overflow-hidden mx-auto relative">
            <motion.div
              animate={{
                x: ["-100%", "100%"],
              }}
              transition={{
                duration: 1.4,
                repeat: Infinity,
                ease: [0.4, 0, 0.2, 1],
              }}
              className="w-1/2 h-full rounded-full bg-gradient-to-r from-transparent via-[#FFD21F] to-transparent shadow-[0_0_6px_#FFD21F]"
            />
          </div>
        )}
      </div>
    </div>
  );
}
