...
import { Sparkles, Star, Quote, ArrowUpRight } from "lucide-react";
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="p-8 sm:p-10 rounded-3xl bg-white border border-slate-200 shadow-card hover:shadow-elevated transition-all flex flex-col justify-between space-y-6 group hover:-translate-y-1"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="px-3 py-1 rounded-full bg-orange-50 text-brand-accent border border-orange-200 font-mono text-xs font-bold">
                    {t.result}
                  </span>
                  <Quote className="w-5 h-5 text-slate-300 group-hover:text-brand-accent transition-colors" />
                </div>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                ...
                />
                <div>
                  <h4 className="font-bold text-sm text-slate-900 font-sans">{t.author}</h4>
