    ...
    <section className="py-14 sm:py-20 bg-[#FAFAFC] text-[#0A0A0E] select-none relative overflow-hidden border-t border-black/5">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-[#0A0A0E] font-display">
        ...
        <div className="space-y-3">
          {faqs.map((faq, index) => {
            ...
            return (
              <div
                key={index}
                className="rounded-2xl bg-white border border-black/8 overflow-hidden shadow-2xs transition-all"
              >
                <button
                  type="button"
                  onClick={() => setOpenIndex(isOpen ? null : index)}
                  className="w-full p-4 sm:p-5 text-left flex items-center justify-between gap-4 select-none hover:bg-black/[0.01] transition-colors"
                >
                  <span className="text-xs sm:text-sm font-bold text-[#0A0A0E] font-sans">
                    {faq.q}
                  </span>
                  <div className="w-7 h-7 rounded-full bg-[#F4F4F8] flex items-center justify-center shrink-0 text-[#0A0A0E]">
                ...
                <AnimatePresence>
                  {isOpen && (
                    ...
                    >
                      <div className="p-4 sm:p-5 pt-0 text-xs sm:text-sm text-[#5A5A68] leading-relaxed font-sans border-t border-black/5">
                        {faq.a}
                      </div>
                    </motion.div>
                  )}
