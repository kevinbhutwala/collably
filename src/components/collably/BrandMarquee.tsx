...
export function BrandMarquee() {
  ...
  return (
    <section className="py-10 border-y border-slate-200 bg-slate-50/70 overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {integrations.map((item, idx) => (
              <div
                key={idx}
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-slate-200/90 text-xs font-medium text-slate-800 shadow-xs select-none"
              >
                <span className="font-bold text-slate-900 font-sans">{item.name}</span>
          ...
          <Link
            href="/for-brands"
            className="inline-flex items-center gap-1 text-xs font-bold text-brand-accent hover:text-orange-700 transition-colors shrink-0 font-mono"
          >
