      ...
      <div className="space-y-6 overflow-x-auto text-[#0A0A0E] dark:text-white">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 min-w-[600px]">
          {creators.map((c) => (
            <div
              key={c.id}
              className="p-5 rounded-3xl bg-[#F8F8FC] dark:bg-white/[0.04] border border-black/8 dark:border-white/10 space-y-4 relative flex flex-col justify-between"
            >
              {onRemoveCreator && (
                <button
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  <div className="relative w-12 h-12 rounded-2xl overflow-hidden bg-black/5 dark:bg-white/[0.05] border border-black/10 dark:border-white/10 shrink-0">
                    <SafeImage
                      src={c.avatarUrl}
                      alt={c.fullName}
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#6A6A78] dark:text-slate-400 font-mono">@{c.handle}</p>
                  </div>
                </div>
                ...
                <div className="p-3.5 rounded-2xl bg-white dark:bg-white/[0.03] border border-black/8 dark:border-white/10 space-y-2 text-xs font-mono">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-emerald-600 dark:text-emerald-400">{c.avgEngagementRate}%</span>
                  </div>
                  <div className="flex justify-between">
