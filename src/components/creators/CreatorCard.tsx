...
export function CreatorCard({ creator }: { creator: CreatorProfile }) {
  return (
    <div className="group rounded-3xl bg-white border border-slate-200 hover:border-slate-300 p-6 shadow-card hover:shadow-elevated transition-all duration-300 hover:-translate-y-1 flex flex-col justify-between">
      <div>
        {/* Top Info */}
          ...
          <div className="flex items-center gap-2 mb-3.5 pt-1">
            {creator.socialAccounts.map((sa) => (
              <span
                ...
                className="p-1.5 rounded-lg bg-slate-50 border border-slate-200/80 text-slate-700 hover:text-slate-900 transition-colors"
              >
